...
select1 = weighted_selection(meal, weight)
while (x != True):
    select2 = weighted_selection(meal, weight)
    if select2 != select1:
        x = True
x = False
while (x != True):
    select3 = weighted_selection(meal, weight)
    if select3 != select1 and select3 != select2:
        x = True

print(f"Your selected meals for the week are the following: {select1}, {select2}, and {select3}")